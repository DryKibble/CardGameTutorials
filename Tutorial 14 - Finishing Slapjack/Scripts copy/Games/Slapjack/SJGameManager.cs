...
public partial class SJGameManager : Node
{
  ...
  [Signal]
  public delegate void WinEventHandler(SJPlayer player);
  [Signal]
  ...
  Timer SlapTimer;
  [Export]
  SJBehavior[] Behaviors;
  SJPlayer[] players = new SJPlayer[Globals.NumCPUs + 1];
  SJPlayer CurPlayer;
  SJPlayer PrevPlayer;
  ...
  private Deal Deal;
  private PackedScene CPU = GD.Load<PackedScene>($"res://Scenes/Games/Slapjack/SJCPU.tscn");

  protected static readonly (Vector3, Vector3)[] positions = [
    (new(0, 0, -1.5f), new(0, -Mathf.Pi,  0)),
    (new(-1.5f, 0, 0), new(0, -Mathf.Pi/2, 0)),
    (new(1.5f, 0, 0), new(0, Mathf.Pi/2, 0)),
    (new(1.5f, 0, -1.5f), new(0, 3*Mathf.Pi/4 ,0)),
    (new(-1.5f, 0, -1.5f), new(0, -3*Mathf.Pi/4 ,0)),
    (new(-1.5f, 0, 1.5f), new(0, -Mathf.Pi/4, 0)),
    (new(1.5f, 0, 1.5f), new(0, Mathf.Pi/4, 0)),
  ];

  protected static readonly int[][] ordering = [
    [0],
    [0, 1],
    [2, 0, 1],
    [2, 3, 0, 1],
  ...
  public override void _Ready()
  {
    ...
    for (byte i = 0; i < ordering[Globals.NumCPUs - 1].Length; i++)
    {
      SJCPU AI = CPU.Instantiate<SJCPU>();
      if (Globals.CPUDifficulty == 4)
      {
        rng.Randomize();
        AI.Behavior = Behaviors[rng.RandiRange(0, 3)];
      }
      else
      {
        AI.Behavior = Behaviors[Globals.CPUDifficulty];
      }
      AI.Name = "CPU" + i;
      (Vector3 pos, Vector3 rot) = positions[ordering[Globals.NumCPUs - 1][i]];
      AI.Position = pos;
      AI.Rotation = rot;
      ...
      AI.Skip += _OnSkip;
      players[i + 1] = AI;
      PlayerCursor.ShowMaterial(AI, (byte)(i + 1));
      AddChild(AI);
      AI.Shockwave.GlobalPosition = Vector3.Zero;

      AI.Shockwave.SetSurfaceOverrideMaterial(0,
        GD.Load<Material>($"res://Art Assets/Textures/SlapShock/SlapShock{i + 1}.tres"));
    }
    ...
    switch (CurState)
    {
      ...
      case GameState.Deal:
        if (Animation.Deal(Deal))
        {
          ...
          PrevPlayer.ShouldSkip = PrevPlayer.iDeck.CurSize == 0;
          CurPlayer = players[numTurns % players.Length];


          while (CurPlayer.CurState == SJPlayer.PlayerState.Out || CurPlayer.ShouldSkip)
          {
            _OnSkip(CurPlayer);
          }
  ...
  private void _OnDeal(SJPlayer player)
  {
    ...
    // -player.Rotation.Y => card's top is to us
    Deal.endRot.Y = -player.Rotation.Y;
    // adds a random amount of rotation so the cards aren't perfectly uniform
    Deal.endRot.Y += rng.RandfRange(-Mathf.Pi / 4, Mathf.Pi / 4);

    // Wrap the value around these -180 and 180 degrees
    // (May be unnecessary)
    // Deal.endRot.Y = Mathf.Wrap(Deal.endRot.Y, -Mathf.Pi, Mathf.Pi);

    Deal.endRot.Z = 0;

    if (!player.IsOut && player.IsMyTurn)
    {
      Deal.dealer = player.iDeck;
      player.iDeck.TopCard.Rotation = Vector3.Zero;
  ...
  private void _OnSlap(SJPlayer player)
  {
    ...
    //and we want no one to be able to Deal
    for (byte i = 0; i < players.Length; i++)
    {
      Deck.ResetPosition(players[i].iDeck);
      Deck.ResetRotation(players[i].iDeck);
      players[i].CanDeal = false;
      players[i].IsMyTurn = false;
    }

    player.Anim.Play("Shockwave");
    slapOrder.Enqueue(player);
    player.CurState = SJPlayer.PlayerState.Out;
    ...
    else
    {
      ...
      if (PrevPlayer is not null)
      {
        if (penalty.iDeck.CurSize != 0)
        {
          ...
          Deal.endRot.X = 0;
          Deal.endRot.Y = -penalty.Rotation.Y + PrevPlayer.Rotation.Y;
          Deal.endRot.Z = Mathf.Pi;
  ...
  private void _ExitReward()
  {
    ...
    if (numOut == players.Length - 1 && allCards)
    {
      ...
      EmitSignal(SignalName.Win, winner);
      PlayerCursor.Glow(winner);
  ...
  private void _OnSkip(SJPlayer player)
  {
    ...
    if (player.CurState == SJPlayer.PlayerState.Out)
    {
      player.IsOut = true;
      PlayerCursor.ShowMaterial(player, 8);
    }
    else
    {
